...
let lastAdClientTime = {}; // لازم يبقى لكل نوع إعلان على حدة (فيديو / خارجي) مش متغير واحد مشترك،
                            // عشان مشاهدة إعلان الفيديو ماتمنعش زرار الإعلان الخارجي من الشغل خالص
...
// بدل ما نكتب الكولداون والمكافأة تابتين هنا وممكن يفرقوا عن قيم السيرفر،
// بنجيبهم من /api/ad_settings ونستخدم نفس القيم اللي السيرفر فعلاً بيحسب بيها.
// لحد ما يوصلوا، بنستخدم قيم افتراضية آمنة (أعلى من أي كولداون متوقع) عشان
// الزرار متفتحش قبل السيرفر يكون جاهز فعلاً يقبل الطلب.
let AD_SETTINGS = {}; // ad_type -> { min_reward, max_reward, cooldown_seconds, max_per_hour }
let adSettingsReady = false;
...
async function loadAdSettings() {
  try {
    ...
    list.forEach((s) => {
      AD_SETTINGS[s.ad_type] = s;
    });
    adSettingsReady = true;
    // نحدّث نص المكافأة على الأزرار عشان يبقى مطابق فعلاً لأقل وأعلى مكافأة ممكنة
    updateAdRewardLabels();
  } catch (e) {
    // لو السيرفر مش راضي يرد، سيبنا القيم المكتوبة في الـ HTML كما هي كـ fallback
  }
}

function updateAdRewardLabels() {
  const map = [
    [els.adBtnVideo, "interstitial"],
    [els.adBtnExternal, "popup"],
  ];
  map.forEach(([btnEl, adType]) => {
    const setting = AD_SETTINGS[adType];
    const rewardEl = btnEl && btnEl.querySelector(".ad-mini-reward");
    if (!setting || !rewardEl) return;
    rewardEl.textContent =
      setting.min_reward === setting.max_reward
        ? `+${setting.min_reward} CCL`
        : `+${setting.min_reward}-${setting.max_reward} CCL`;
  });
}

function adCooldownMs(adType) {
  const setting = AD_SETTINGS[adType];
  // fallback 60 ثانية (نفس default في db.py) لو الإعدادات لسه ما وصلتش
  const seconds = setting ? setting.cooldown_seconds : 60;
  return seconds * 1000;
}
...
function startCooldownDisplay(btnEl, labelEl, originalLabel, cooldownMs) {
  let secondsLeft = Math.ceil(cooldownMs / 1000);
  btnEl.disabled = true;
...
function setupAdButton(btnEl, adType, sdkArg) {
  ...
  btnEl.addEventListener("click", async () => {
    const cooldownMs = adCooldownMs(adType);
    const now = Date.now();
    if (now - (lastAdClientTime[adType] || 0) < cooldownMs) return;
    ...
    call
      ...
      .catch(() => {
        els.adOverlay.classList.add("hidden");
        btnEl.disabled = false;
        lastAdClientTime[adType] = 0; // اسمحله يجرب تاني على طول لو فشل
      });
  });
}
...
async function finishAdWatch(adType, btnEl, labelEl, originalLabel) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      startCooldownDisplay(btnEl, labelEl, originalLabel, cooldownMs);
      return;
    }

    // مهم: السيرفر رفض يدي عملات (كولداون لسه شغال أو تجاوزت الحد بالساعة).
    // من قبل كنا بنتجاهل الحالة دي بالكامل فالمستخدم يتفرج على الإعلان
    // وما ياخدش عملات من غير ما يعرف السبب - ده كان سبب مشكلة
    // "الإعلانات ساعات مش بتحسب عملات".
    if (tg && tg.HapticFeedback) tg.HapticFeedback.notificationOccurred("error");

    if (state && state.error === "cooldown") {
      alert(`لسه لازم تستنى ${state.seconds_left} ثانية قبل ما تتفرج على إعلان تاني 🙏`);
      startCooldownDisplay(btnEl, labelEl, originalLabel, state.seconds_left * 1000);
      return;
    }

    if (state && state.error === "hourly_limit") {
      alert(`وصلت للحد الأقصى من الإعلانات في الساعة دي، جرب تاني بعد ${Math.ceil(state.seconds_left / 60)} دقيقة 🙏`);
      startCooldownDisplay(btnEl, labelEl, originalLabel, state.seconds_left * 1000);
      return;
    }

    alert("حصل خطأ وإحنا بنسجل مشاهدتك للإعلان، جرب تاني كمان شوية 🙏");
    btnEl.disabled = false;
  } catch (e) {
    alert("حصل خطأ في الاتصال، جرب تاني كمان شوية 🙏");
    btnEl.disabled = false;
  }
}
