...
def api_tasks():
    ...
    return jsonify(db.get_tasks_status(user_id))
...
def api_tasks_claim():
    ...
    result = db.claim_task(user_id, task_id)
    if "error" in result:
...
def admin_update_user(user_id):
    fields = [
        "first_name", "coins", "energy", "max_energy",
        "total_taps", "ads_watched", "referral_count", "referral_coins",
    ]
    updates = {field: request.form.get(field) for field in fields if field in request.form}
    try:
        updated = db.update_user_by_admin(user_id, updates)
        if updated is None:
            flash("اللاعب غير موجود.", "error")
        else:
            flash(f"تم تحديث بيانات اللاعب {updated['first_name'] or user_id} بنجاح.", "success")
    except ValueError as exc:
        flash(str(exc), "error")
    return redirect(url_for("admin_dashboard"))


@app.route("/admin/users/<int:user_id>/reset-tasks", methods=["POST"])
@admin_required
def admin_reset_user_tasks(user_id):
    if db.reset_user_tasks_by_admin(user_id):
        flash("تم إعادة ضبط مكافآت المهام لهذا اللاعب.", "success")
    else:
        flash("اللاعب غير موجود.", "error")
    return redirect(url_for("admin_dashboard"))
