...
def init_db():
    ...
    with _lock, _get_conn() as conn:
        ...
        for ad in default_ads:
            ...
            # الداتابيز ممكن تكون اتعملها seed قبل كده بقيم قديمة (60 ثانية / مكافأة عشوائية)،
            # فبنحدّث القيم الأساسية دايماً عشان تفضل مطابقة لنفس القيم دي بالظبط،
            # من غير ما نلمس "active" لو حد عطّل النوع ده يدوي من الأدمن.
            conn.execute(
                """
                UPDATE ad_settings
                SET
...
def get_ad_settings():
    """
    بيرجع إعدادات الإعلانات النشطة (المكافأة، الكولداون، الحد بالساعة)
    عشان الفرونت إند يعرض نفس القيم اللي السيرفر بيحسب بيها فعلاً،
    بدل ما تبقى القيم مكررة ومكتوبة يدوي في الـ HTML/JS وتفرق عن قيم الداتابيز.
    """
...
def watch_ad(
    ...
    with _lock, _get_conn() as conn:
        ...
        hour_count = len(hour_ads)

        if hour_count >= setting["max_per_hour"]:

            # الوقت المتبقي لحد ما أقدم إعلان في الساعة يخرج بره النافذة
            # (مش دايماً 3600 ثانية كاملة - ده كان بيدي وقت أطول من المفروض)
            oldest_watched_at = hour_ads[0]["watched_at"]
