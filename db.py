...
# على Railway استخدم Volume وثبّت DATA_DIR=/app/data.
# محليًا، لو DATA_DIR غير موجود، القاعدة تفضل بجانب المشروع.
...
def _get_conn():
    # تأكد إن مجلد التخزين الدائم موجود قبل فتح SQLite.
...
def update_user_by_admin(user_id: int, updates: dict):
    """تعديل بيانات لاعب من لوحة الأدمن مع التحقق من القيم."""
    allowed = {
        "first_name",
        "coins",
        "energy",
    ...
    for key, value in updates.items():
        if key not in allowed:
            continue
        if key == "first_name":
            clean[key] = str(value).strip()[:100]
        else:
            try:
                clean[key] = max(0, int(value))
            except (TypeError, ValueError):
                raise ValueError(f"قيمة غير صالحة: {key}")

    if not clean:
        raise ValueError("لا توجد بيانات صالحة للتعديل")

    if "max_energy" in clean:
        clean["max_energy"] = max(1, clean["max_energy"])
...
def reset_user_tasks_by_admin(user_id: int):
    """إلغاء كل المكافآت التي تم استلامها ليتمكن اللاعب من استلامها مجددًا."""
    with _lock, _get_conn() as conn:
        row = conn.execute("SELECT user_id FROM users WHERE user_id=?", (user_id,)).fetchone()
        if row is None:
            return False
        conn.execute(
            "UPDATE users SET claimed_tasks='', last_active=? WHERE user_id=?",
            (time.time(), user_id),
        )
        conn.commit()
        return True
